extends "res://Scripts/Weapons/GunSights.gd"
...
func sight(var x_rotation: float, var y_rotation: float):
	...
	var cannon_bone = $Armature/Bone/Bone001
	if cannon_bone:
		cannon_bone.rotation_degrees.z = -90 + y_rotation
	var base_bone = $Armature/Bone
	if base_bone:
		base_bone.rotation_degrees.y = x_rotation
...
func reset_sights():
	...
	var gun_bone = $Armature/Bone/Bone001
	if gun_bone:
		gun_bone.rotation_degrees.x = -90
	var turret_bone = $Armature/Bone
	if turret_bone:
		match position:
			"North":
